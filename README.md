...
카탈로그(Consumer → Provider DSP) 데모는 `docker compose up` 후 `python scripts/catalog_demo.py` 로 재현할 수 있습니다.
...
- `scripts/catalog_demo.py` — Provider에 자산·계약 시드 후 Consumer에서 `CatalogRequest` 호출
