...
export class AppComponent implements AfterViewInit {
  ...
  async ngAfterViewInit(): Promise<void> {
    ...
    this.state.currentEdcConfig$
      ...
      .subscribe(edc => {
        ...
        this.redirectIfConsumerOnProviderRoute(edc);
      });
  }
  ...
  private filterMenuForConnector(base: AppConfig, edc: EdcConfig | undefined): AppConfig {
    ...
    return {
      ...base,
      menuItems: base.menuItems.filter(item => !PROVIDER_ONLY_ROUTES.has(item.routerPath)),
    };
  }

  private redirectIfConsumerOnProviderRoute(edc: EdcConfig | undefined): void {
    const isConsumer = (edc?.connectorName ?? '').toLowerCase() === 'consumer';
    if (!isConsumer) {
      return;
    }
    ...
    if (top && PROVIDER_ONLY_ROUTES.has(top)) {
